    ...
    var initClass = function (config) {
        ...
        function setupAdmin(callback) {
            db.run("INSERT OR IGNORE INTO admin VALUES (?,?,?,?)",['admin',getRandomKey(8,36), 1,0], function (err) {
                if (err) console.log("Error in setupAdmin(): "+e);
                callback();
            });
        };
        ...
        function getNewSchemaVersion () {
            var sql = fs.readFileSync('./resource/schema.sql').toString();
        ...
        function showUrl () {
            ...
            db.all('SELECT adminID,name,adminKey,role FROM admin',[],function(err,rows){
                for (var i=0,ilen=rows.length;i<ilen;i+=1) {
                    ...
                    config.admin[row.adminKey] = {name:row.name,role:row.role,id:row.adminID};
                    if (row.role == 1) {
