(function () {
    var initClass = function (config) {
        ...
        function showUrl () {
            db.all('SELECT name,adminID,role FROM admin',[],function(err,rows){
                for (var i=0,ilen=rows.length;i<ilen;i+=1) {
                    ...
                    if (row.name === 'admin') {
                        console.log("Admin URL: http://" + config.proxy_hostname + port + '/?admin=' + row.adminID);
                    }
                    config.admin[row.adminID] = {name:row.name,role:row.role};
                    if (row.role == 1) {
                        console.log("Adding admin role");
                    } else if (row.role == 2) {
                        console.log("Adding commenter '" + row.name + "' with URL http://" + config.proxy_hostname + port + '/?commenter=' + row.adminID);
                    }
                }
            });
        ...
        function saveRubric (classID,quizNumber,questionNumber, dataIn) {
            ...
            questionLoadCount += 1;
            db.run('INSERT OR IGNORE INTO strings VALUES (NULL,?)',[dataIn.rubric],function(err){
                if (err) console.log("OUCH(1)! "+err);
        ...
        function saveChoice (classID,quizNumber,questionNumber,choiceNumber, dataIn, dataOut) {
            ...
            db.run('INSERT OR IGNORE INTO strings VALUES (NULL,?)',[str],function(err){
        ...
        function saveAnswer (classID, quizNumber, questionNumber, studentID, choice) {
            questionLoadCount += 1;
            if (!remap.students[studentID]) {
                console.log("Warning: StudentID not in remap: "+studentID);
                questionLoadCount += -1;
                if (!questionLoadCount){
