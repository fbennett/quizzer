(function () {
    ...
    cogsClass.prototype.loadCog = function (cogModule,groupName,pageName,callName) {
        ...
        if (!cogClass.match) {
            cogClass.match = this.sys[groupName + 'CallValidator'](pageName,callName);
        }
    ...
    cogsClass.prototype.getCogs = function () {
        ...
        for (var k=0,klen=groups.length;k<klen;k+=1) {
            ...
            for (var i=0,ilen=pages.length;i<ilen;i+=1) {
                ...
                try {
                    cogModule.cogClass.prototype.altpage = this.sys.fs.readFileSync('./api/' + groupName + '/' + pageName + '/altpage.html');
                } catch (e) {};
                this.loadCog(cogModule,groupName,pageName);
                
                var calls = this.sys.fs.readdirSync('./api/' + groupName + '/' + pageName);
                for (var j=0,jlen=calls.length;j<jlen;j+=1) {
                    var callName = calls[j];
                    if (['altpage.html','matchExec.js','page.html','altpage.html~','matchExec.js~','page.html~'].indexOf(callName) > -1) continue;
                    cogModule = require('../api/' + groupName + '/' + pageName + '/' + callName + '/matchExec.js');
                    this.loadCog(cogModule,groupName,pageName,callName);
                }
            }
        }
