(function () {
    ...
    optsClass.prototype.getOpts = function () {
        ...
        // Write commenter message template to disk if necessary
        fs.readFile("commenter-message.txt",function(err) {
            if (err) {
                var commenterMessage = fs.readFileSync(__dirname + "/../resource/commenter-message.txt").toString();
                fs.writeFileSync('commenter-message.txt',commenterMessage);
            }
        });
        return this.config;
    }
