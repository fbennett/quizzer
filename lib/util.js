        ...
        return {
            writeQuestion: function (response,classID,quizNumber,questionNumber,fields) {
                ...
                function saveQuestion (classID,quizNumber,questionNumber,dataOut) {
                    sys.db.get('SELECT questionID FROM questions WHERE classID=? AND quizNumber=? AND questionNumber=?',[classID,quizNumber,questionNumber],function(err,row){
                        if (err) {return oops(response,err,'writeQuestion')};
                        if (row && row.questionID) {
                            saveQuestionInsert(row.questionID,classID,quizNumber,questionNumber,dataOut);
                        } else {
                            saveQuestionInsert(null,classID,quizNumber,questionNumber,dataOut);
                        }
                    });
                };
                function saveQuestionInsert(questionID,classID,quizNumber,questionNumber,dataOut) {
                    sys.db.run('INSERT OR REPLACE INTO questions VALUES (?,?,?,?,?,?,?,?,?,?)',[questionID, classID, quizNumber, questionNumber, dataOut[5], dataOut[0], dataOut[1], dataOut[2], dataOut[3], dataOut[4]],function(err){
                        if (err) {return oops(response,err,'saveQuestionInsert()')};
                    });
                };
            },
