        ...
        sys.db.all('SELECT adminID,name,email,interval FROM admin WHERE email IS NOT NULL AND interval IS NOT NULL AND role=2',[],function(err,rows){
            ...
            if (rows) {
                for (var i=0,ilen=rows.length;i<ilen;i+=1) {
                    ...
                    updateCommenterKeys(adminID,name,email,adminKey,dow);
                }
            }

            function updateCommenterKeys (adminID,name,email,adminKey,dow) {
                sys.db.run('UPDATE admin SET adminKey=? WHERE adminID=?',[adminID],function(err){
                    if (err) {return 'Error in scheduler(2)'};
                    scheduleMail(adminID,name,email,adminKey,dow);
                });
            };
                
            function scheduleMain(adminID,name,email,adminKey,dow) {
                // var hostname 
