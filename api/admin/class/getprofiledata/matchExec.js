(function () {
    ...
    cogClass.prototype.exec = function (params, request, response) {
        ...
        var sql = 'SELECT students.studentID,'
            ...
            +   'FROM answers'
            + ') AS total ON total.studentID=memberships.studentID AND total.classID=memberships.classID '
            + 'LEFT JOIN ('
            +   'SELECT answers.questionID,answers.studentID '
            +   'FROM questions '
            +   'JOIN answers USING(questionID) '
            +   'WHERE choice=correct'
            + ') AS correct ON correct.studentID=memberships.studentID AND correct.classID=memberships.classID '
